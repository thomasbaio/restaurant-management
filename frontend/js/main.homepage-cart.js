// main.js — aggiornato per homepage con carrello (cart_home_v2)
...
function pickImageURL(p) {
  ...
  const label = encodeURIComponent((p.nome || p.name || "Food").split(" ")[0]);
...
/* ============== integrazione con carrello homepage (cart_home_v2) ============== */

const CART_KEY = "cart_home_v2";

// prova a determinare il nome ristorante (best-effort)
async function restaurantNameById(rid) {
  if (!rid) return "";
  try {
    const res = await fetch(`${API_BASE}/users/restaurants`, { mode: "cors" });
    if (!res.ok) return "";
    const arr = await res.json();
    const r = (Array.isArray(arr) ? arr : []).find(u => String(u.restaurantId ?? u.id ?? u._id ?? "") === String(rid));
...
function writeCart(cart) {
  ...
  // aggiorna badge subito (home-cart.js lo farà anche al prossimo sync)
...
// Espone una funzione globale per aggiungere al carrello dal resto del sito
window.homeCartAdd = async function homeCartAdd(meal) {
  const user = JSON.parse(localStorage.getItem("loggedUser") || "null");
  if (!user || user.role !== "cliente") {
    alert("Devi accedere come cliente per aggiungere al carrello.");
    return;
  }
  const id = meal.id || meal.idmeals || meal.raw?._id || `${meal.nome}|${meal.restaurantId}|${meal.prezzo}`;
  let cart = readCart();
  let line = cart.find(x => x.id === id);
  if (line) {
    line.qty = (Number(line.qty)||0) + 1;
  } else {
    const rName = await restaurantNameById(meal.restaurantId);
    line = {
      id,
      name: meal.nome || "Piatto",
      price: Number(meal.prezzo || 0),
      image: pickImageURL(meal),
      restaurantId: meal.restaurantId || "r_o",
  ...
  writeCart(cart);
  // feedback UX
  const openBtn = document.getElementById("openCartBtn") || document.getElementById("cartFab");
  if (openBtn) openBtn.classList.add("pulse");
  setTimeout(() => openBtn && openBtn.classList.remove("pulse"), 600);
  // opzionale: apri subito il carrello
  // document.getElementById("openCartBtn")?.click();
};

// compat: se esisteva già window.addToCart, reindirizza al nuovo flusso
window.addToCart = function addToCart(meal) {
  // normalizza minimi
  const m = {
    ...meal,
    id: meal.id || meal.idmeals || meal.raw?._id,
    nome: meal.nome || meal.name || meal.strMeal || "Piatto",
    prezzo: (typeof meal.prezzo === "number") ? meal.prezzo : Number(meal.prezzo || meal.price || 0),
    restaurantId: meal.restaurantId || null
  };
  return window.homeCartAdd(m);
};
    ...
    for (const [, g] of entries) {
      ...
      for (const meal of g.items) {
        ...
        if (user?.role === "cliente") {
          ...
          btn.addEventListener("click", () => window.homeCartAdd(meal));
